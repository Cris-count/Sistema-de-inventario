    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-1">
      @for (p of plans(); track p.codigo) {
        <button
          ...
          class="w-full rounded-2xl border border-slate-200 bg-surface p-4 text-left transition hover:border-teal-200 hover:shadow-sm"
        ...
        >
          ...
          @if (p.descripcion) {
            <p class="mt-1 text-sm text-secondary">{{ p.descripcion }}</p>
          }
          ...
          <p class="mt-2 text-xs text-secondary">Hasta {{ p.maxBodegas }} bodegas · hasta {{ p.maxUsuarios }} usuarios</p>
        </button>
      }
    </div>
