  ...
  template: `
    <div class="page stack">
      ...
      <form [formGroup]="form" (ngSubmit)="submit()" class="card stack">
        ...
        <button type="submit" class="btn btn-primary" [disabled]="saving()">Registrar salida</button>
      </form>
    </div>
  `
})
export class MovSalidaPage {
  ...
  submit(): void {
    ...
    type Linea = { productoId: number | null; bodegaOrigenId: number | null; cantidad: string };
    ...
    this.api
      ...
      .subscribe({
        next: (m) => {
          ...
          this.lines.clear();
          this.addLine();
          this.form.patchValue({ motivo: '', referenciaDocumento: '', observacion: '' });
        },
        error: (e) => {
          this.message.set(null);
