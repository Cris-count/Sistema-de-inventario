...
public class VentaController {
    ...
    @PostMapping("/stripe/preparar")
    @PreAuthorize("hasAnyAuthority('VENTAS','ADMIN','SUPER_ADMIN')")
    @Operation(summary = "POS: crea venta PENDIENTE_PAGO y sesión Stripe Checkout (sin movimiento hasta cobro confirmado)")
    public VentaStripePrepararResponse prepararStripeCheckout(@Valid @RequestBody VentaCreateRequest req) {
        return ventaPosStripeService.prepararCheckoutStripe(req);
    }

    @PostMapping("/{id}/stripe/sincronizar")
    @PreAuthorize("hasAnyAuthority('VENTAS','ADMIN','SUPER_ADMIN')")
    @Operation(summary = "POS: consulta Stripe con session_id y finaliza venta si el pago está pagado (respaldo al webhook)")
    public VentaDetailResponse sincronizarStripe(
            @PathVariable Long id, @Valid @RequestBody VentaStripeSyncRequest body) {
        return ventaPosStripeService.sincronizarPago(id, body);
    }

    @PostMapping("/{id}/cancelar-pendiente")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @PreAuthorize("hasAnyAuthority('VENTAS','ADMIN','SUPER_ADMIN')")
    @Operation(summary = "POS: cancela una venta pendiente de pago Stripe (sin movimiento de inventario)")
    public void cancelarPendiente(@PathVariable Long id) {
        ventaPosStripeService.cancelarVentaPendiente(id);
    }
